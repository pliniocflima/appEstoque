...
import { LayoutDashboard, ShoppingCart, ShoppingBag, Utensils, Settings, LogOut, Menu, X, Sliders, History, BarChart3, ClipboardCheck } from 'lucide-react';
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden">
        ...
        <nav className="p-4 space-y-2">
          {navItems.map((item) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center px-4 py-3 rounded-lg transition-colors ${isActive ? 'bg-blue-50 text-blue-600 font-medium' : 'text-gray-600 hover:bg-gray-50'}`
              }
            >
          ...
          <button onClick={handleLogout} className="flex w-full items-center px-4 py-3 text-gray-600 hover:bg-red-50 hover:text-red-600 rounded-lg transition-colors mt-8">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 pb-20"><Outlet /></div>
      </main>
    </div>
  );
};
